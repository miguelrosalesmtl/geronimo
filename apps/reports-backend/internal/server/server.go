...
import (
	...
	"github.com/miguelrosalesmtl/geronimo/apps/reports-backend/internal/settings"
...
func New(
	...
	testrunsService *testruns.Service,
	pool *pgxpool.Pool,
	log *slog.Logger,
) *Server {
	s := &Server{
...
func (s *Server) routes() http.Handler {
	...
	r.Route("/api/v1", func(r chi.Router) {
		...
		r.Group(func(r chi.Router) {
			...
			// Test runs. The e2e suite's API key POSTs here after every
			// Playwright run; testruns.read browses what it submitted.
			r.With(s.requirePermission(identity.PermTestRunsCreate)).
				Post("/test-runs", s.handleCreateTestRun)
			r.With(s.requirePermission(identity.PermTestRunsRead)).
				Get("/test-runs", s.handleListTestRuns)
			r.With(s.requirePermission(identity.PermTestRunsRead)).
				Get("/test-runs/{runID}", s.handleGetTestRun)
			r.With(s.requirePermission(identity.PermTestRunsRead)).
				Get("/test-runs/{runID}/results", s.handleListTestRunResults)
		})
	})
