...
import (
	...
	"github.com/miguelrosalesmtl/geronimo/apps/reports-backend/internal/mail"
...
func cleanDatabase(t *testing.T) {
	...
	err := database.InTx(ctx, testPool, func(db database.DB) error {
		...
		for _, stmt := range []string{
			...
			// test_runs.created_by references users; internal/testruns' own
			// suite shares this database (see suiteLockKey) and leaves rows
			// behind for its own tests to see, so this suite must clear them
			// too before it can delete users.
...
func newHarnessWith(t *testing.T, rl settings.RateLimit) *harness {
	...
	testrunsSvc := testruns.NewService(testPool, slog.New(slog.NewTextHandler(io.Discard, nil)))
