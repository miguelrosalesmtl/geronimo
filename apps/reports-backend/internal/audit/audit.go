...
const (
	...
	// --- Test runs ---
	// Submitted by the e2e suite's API key after each Playwright run. The actor
	// is the key's owning user; RequestMeta.APIKeyID (see below) is what marks
	// it as automation rather than a human posting the same payload by hand.
	ActionTestRunCreated Action = "testruns.created"
